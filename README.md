...
## Why a glass-box engine is the verdict-of-record (design choice, not a shortcut)

Prahari deliberately uses an **explainable rule/signal engine as the verdict of record**,
with learned models layered *on top of* it rather than replacing it. The reason is the
evaluation criterion itself — *"auditability of intelligence packages for legal
admissibility."*

> An intelligence package that ends up in court must survive the question **"why was this
> flagged?"** A glass-box engine answers at phrase level: *this exact sentence triggered
> authority-impersonation, weighted 18, contributing 15.8% of the score.* A black-box
> transformer cannot answer that without a separate, contestable explainability layer —
> and a defence counsel can attack a saliency map far more easily than a quoted phrase.
> So the auditable core produces the verdict and the hash-sealed evidence trail, while the
> **optional LLM second opinion** (`backend/llm.py`) and the **CNN/ViT counterfeit roadmap**
> add pattern-detection breadth *on top of* it, never instead of it.

This also gives exact **additive attribution** — because the model is linear, each signal's
contribution is its true Shapley value, no SHAP/LIME approximation required — and it means
a new scam template is added by writing one auditable pattern, not by retraining and
re-validating a whole model.
...
| 5 | **Citizen Fraud Shield** | Two modes for the citizen, in **their own language end-to-end**. **(a) Check a message** — instant verdict, plain-language reasons and a guided 1930 / cybercrime.gov.in report. **(b) Live Call Guard** — protection *during* the call: the victim taps what the caller is doing, each tactic reveals the exact counter-instruction, and if money has already moved a **60-minute golden-hour countdown** starts with a one-tap 1930 call and the script to read out. A free-text box handles scripts not in the checklist by running the real classifier. Languages offered are only those **fully authored end-to-end** (English, हिन्दी, தமிழ், বাংলা) — the detector also reads Devanagari, so a Hindi user can type in Hindi; we don't list a language we can't answer in. |
...
- Every miss is shown openly in the **Honest misclassifications** panel (no cherry-picking).

### Held-out test — independently sourced (the number that matters)

A perfect score on a corpus this repo generates would be circular, so we also run a
**separate, held-out set** (`backend/heldout.py`, surfaced live on the Model Performance
page): scam scripts **paraphrased from publicly documented digital-arrest / fraud cases**
(MHA–I4C advisories and mainstream reporting) plus genuine hard negatives. **None of these
phrasings were used to author the detector's patterns** — it is a true out-of-sample test.

| | Precision | Recall | Accuracy | False alarms |
|---|:--:|:--:|:--:|:--:|
| In-house corpus (96 msgs) | 100% | 100% | 100% | 0% |
| **Held-out, independently sourced (20 msgs)** | **100%** | **91.7%** | **95%** | **0%** |

We report the lower number deliberately: an independently-sourced 91.7% is more credible
than a self-graded 100%, and the single miss (a vague electricity-disconnection SMS with
no link or payment demand) is visible rather than hidden.
