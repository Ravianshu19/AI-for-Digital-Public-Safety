...
| 5 | **Citizen Fraud Shield** | Conversational, **low-false-positive** assistant that answers **entirely in the citizen's language** — select हिन्दी and the greeting, verdict, reasons, next steps and golden rules are all pure Hindi (the detector also reads Devanagari scam scripts, so a Hindi user can type in Hindi). **4 languages fully authored** (English, हिन्दी, தமிழ், বাংলা); the other 8 of the 12 listed fall back to English until verified native strings are added — machine-guessed translations are deliberately not shipped. Gives an instant verdict and a **guided 1930 / cybercrime.gov.in report**. Channel-agnostic: WhatsApp Business API and IVR call the same `/api/shield/assess` endpoint (integration-ready; this repo ships the web/app prototype). |
