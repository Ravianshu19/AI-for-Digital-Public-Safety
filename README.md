...
**External real-data validation** (`GET /api/eval/external`): run against the **UCI SMS
Spam Collection** (5,574 real SMS), the false-positive rate on the **4,827 genuine
messages is 0.10%** (5 flagged) — the low-false-positive claim, proven on real data at
scale. (Recall on that corpus is out-of-domain — generic UK SMS spam, not Indian scam
families — so true recall is the 97.1% in-domain figure above.)
...
| External validation | **Real UCI SMS Spam Collection** (5,574 SMS) — `sample_data/sms_spam/` | `backend/external_eval.py` → 0.10% FPR on real ham |
