...
The scam classifier is benchmarked **live** against a **92-message corpus across 12
scam categories** (digital-arrest, UPI collect-request, OTP/credential phishing,
KYC/suspension, lottery, loan, investment/crypto, fake-job, parcel, sextortion,
romance/matrimony) plus genuine **hard-negative** messages. Metrics are computed on
every page load (`GET /api/eval/metrics`) — nothing is pre-baked. The synthetic corpus
is generated by `backend/scam_corpus.py` (schema: `call_text, label, agency_claimed, risk`).

| Precision | Recall | F1 | Accuracy | False-positive rate |
|:---:|:---:|:---:|:---:|:---:|
| **100%** | **96.9%** | **98.4%** | **97.8%** | **0.0%** |
